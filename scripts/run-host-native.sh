...
SECURITY_CONFIG="${SECURITY_CONFIG:-$ROOT_DIR/config/security.host-native.yaml}"
...
mkdir -p "$BIN_DIR" "$LOG_DIR" "$DEMO_DATA_DIR" "$RUSTFS_DATA_DIR"
...
Host-native stack is running without ClamAV.
